...
export const getProfile = cache(async () => {
  ...
  const { data: profile, error } = await supabase
    .from("profiles")
    .select(
      "id, role, display_name, avatar_url, banner_id, avatar_frame_id, avatar_skin_id, coins_bonus, coins_spent, xp, level, duel_rating, duel_wins, duel_losses, personal_email, institutional_email, must_change_password, profile_completed, is_master",
    )
    .eq("id", user.id)
    .single();
